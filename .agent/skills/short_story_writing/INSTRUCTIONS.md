...
## Overview
「単一の効果 (Single Effect)」を最大化し、無駄を削ぎ落とした鋭い物語を作ることを目的とする。
...
### 5. 散文密度・Natural Prose・執筆ルール

→ Author DNA §3（散文密度）、§4（Natural Prose）、§7（禁止事項）を遵守。
- **メタ発言禁止・Ep.X参照禁止・SDT**: → Author DNA §7 および novel_writing INSTRUCTIONS.md の Writing Rules に準拠。
- **File Overwrite Safety**: 既存ファイルを `Write` で上書きする前に、必ず `Read` で現在の内容を確認する。
