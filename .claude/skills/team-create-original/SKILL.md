...
- **ログライン**: What If + 主人公 + 対立 + Stakes を含む一文
- **プレミス**: ログラインを発展させ、テーマの方向性を示すもの
...
- **Ghost → Lie → Want vs Need チェーン**（Weiland）:
  - **Ghost**: キャラクターが「嘘」を信じる原因となった過去の出来事
  - **Lie**: Ghost のせいで信じ込んでいる誤った信念
  - **Want**: Lie に基づいて本人が自覚している目標
  - **Need**: Lie を克服した先にある本当の課題
- **Character Web**（Truby）:
...
- **Stakes の階層**（Dramatica）:
  - **個人的 Stakes**: 主人公個人が失うもの
  - **関係的 Stakes**: 人間関係に生じる損害
  - **社会的 Stakes**: 世界やコミュニティへの影響
  - **存在論的 Stakes**: テーマレベルの賭け
- **失敗の帰結**: 主人公が失敗したら何が起きるか
- **成功の代償**: 主人公が成功しても何を失うか
...
- 各キャラクターの「テーマへの回答」が、テーマの異なる側面を照らしているか検証する
- テーマが抽象的すぎれば具体化し、狭すぎれば拡張する
- **最終的なテーマ文を一文で確定する**
- **Designing Principle**（Truby）: テーマを体現する**構造的仕掛け**を言語化する
...
- **Tone Statement**: この物語の感情的温度を一文で宣言する — Writer と Editor の判断基準を揃えるために必須
...
### 2i. タイムライン作成（Timeline）— 連載の場合

- `.agent/templates/timeline_template.md` を参照する
- 物語開始時点（年月/季節）、時間の流れ方を定義する
- バックストーリーの時系列を整理する
- 出力: `docs/timeline.md`

### 2j. スタイルガイド作成（Style Guide）

- `.agent/templates/style_guide_template.md` を参照する
- 固有名詞辞書、表記ルール（ダッシュ、三点リーダ等）、POV別文体差を定義する
- 出力: `docs/style_guide.md`

### 2k. ミステリー/事件テンプレート準備（Mystery Series の場合）

- `.agent/templates/mystery_design_template.md` を参照する
- 事件カテゴリ表を作成する（技術/人間/AI/社会/存在論 等、作品に応じて）
- 事件×心理テーマの接続ルールを定義する
- 「犯人/原因側からの逆算」「二重読者体験（連載追い vs 一気読み）」を設計原則に含める
- 出力: `docs/world/event_categories.md`

**Checkpoint 2**（最重要）: ジャンル契約・プレミス・世界・キャラクター（ウェブ）・対立（ステークス）・テーマ・語り・仮題・タイムライン・スタイルガイドをユーザーが承認する。ここから先の全てはこの基盤の上に構築されるため、慎重に確認する。
...
- 幕構成を設計する（典型的には 3〜4幕。長期連載はアーク単位）
- 各幕/アークをエピソードに分割し、一行サマリーを書く
- 鍵となるターニングポイント、ミッドポイントリバーサル、クライマックスを特定する
- 伏線スレッドを設計する
- **Web連載の鉄則**: 「セットアップだけの話は1話も許されない」。毎話にペイオフを入れる
- **ミステリー連載**: 各事件に `.agent/templates/mystery_design_template.md` を適用し、犯人視点から逆算して手がかりを配置する
- 出力:
...
3. **作品の美学** — Phase 2c の Sensory Palette + 仮題から導出
4. **トーンと語り** — Phase 2g の語り設計をそのまま転記
5. **テーマ** — Phase 2f で精緻化されたテーマ文 + Designing Principle
6. **世界設定** — `docs/world/` への参照リンク付きで要約
7. **キャラクター・ダイナミクス** — Character Web マトリクス・Ghost/Lie 構造・関係性の力学を要約
...
市場調査と題材調査は独立した作業。だが Phase 2 内は依存関係がある: ジャンル定義→プレミス→世界→キャラ→対立→テーマ精緻化→語り→仮題。
